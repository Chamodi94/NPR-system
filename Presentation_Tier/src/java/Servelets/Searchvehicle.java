    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           if(request.getParameter("btnsearch")!= null){
               List<Vehicle_Info> vehicle=new ArrayList<Vehicle_Info>();
               vehicle.addAll((List)proxy.searchVehicleInfo(vehicleno));
               if(vehicle.isEmpty()){
                   request.setAttribute("result", "failed");
               }
               else{
               request.setAttribute("Vehicleinfo", proxy.searchVehicleInfo(vehicleno));
               }
           }
            request.getRequestDispatcher("Search_Vehicles.jsp").forward(request, response);
        }
    }
